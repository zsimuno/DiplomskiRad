...
\begin{document}
	...
	\begin{intro}
		...
		Premda su najveæe i najpoznatije igre današnjice najèešæe 3D, 2D je još uvijek prisutan i èesto ga susreæemo, pogotovo u svijetu nezavisnih (indie) igara. Svijet 2D-a prepun je moguænosti, pa se takve igre neæe tako skoro prestati programirati. Pojavom modernijih, objektno orijentiranih jezika došle su i biblioteke koje omoguæavaju pisanje koda za igre. Ti jezici i biblioteke dopuštaju brzo, efikasno i pregledno pisanje koda za igre. 
		
		Jedna od takvih biblioteka za jezik C++ i 2D igre je Simple and Fast Multimedia Library ili SFML. Ona donosi moæne alate za programiranje igara koji omoguæavaju jednostavno i brzo programiranje.
		...
		Ovaj diplomski rad prikazuje biblioteku SFML, neke njezine osnovne dijelove i moguænosti. Specifièno se osvræe i na neke oblikovne obrasce koji su posebno efikasni u svijetu programiranja igara te ih prikazuje s problemima koje oni rješavaju. Te obrasce gledat æemo kako nam omoguæavaju pisanje kvalitetnog i efikasnog koda za videoigre.
	\end{intro}
	...
	\chapter{SFML}	
	Kada je u pitanju programiranje igara u 2D-u, imamo razlièite moguænosti u razlièitim jezicima. Neke biblioteke nam nude više, a neke manje moguænosti. U nekima možemo upravljati dijelovima programa bolje nego u drugima. Jedna od biblioteka koja dopušta više moguænosti je Simple and Fast Multimedia Library, odnosno SFML. To je takoðer biblioteka za jezik C++ koji sam po sebi omoguæava preciznije upravljanje objektima i memorijom koju oni zauzimaju. 
	...
	Kao što joj samo ime govori (Jednostavna i brza biblioteka), SFML biblioteka jednostavna je za korištenje i omoguæava brze programe i brzo pisanje kodova. Nudi jednostavno aplikacijsko programsko suèelje (engl. application programming interface ili API) koje je pregledno i pristupaèno za korisnike. 
	...
	Pogledati æemo jedan osnovan primjer programa pisanog u SFML-u i u komentarima ukratko objasniti što se dogaða.
	...
	\end{enumerate}
	Unose èitamo korištenjem klase \lstinline{sf::Event} preko koje odreðujemo dogaðaje koje je korisnik pokrenuo. Ti æe dogaðaji najèešæe utjecati na objekte na zaslonu, bilo to pomicanje objekata ili neke radnje (napad, obrana, magija i sl.). Te æemo objekte crtati u novim (ili istim ako nije bilo promjene) stanjima. Za crtanje u prozoru koristi se Double buffer oblikovni obrazac o kojem æe biti govora malo poslije. Ukratko, dok je nešto nacrtano u prozoru, u pozadini pripremamo ono što se crta sljedeæe i kada doðe vrijeme, izbrišemo trenutno stanje i nacrtamo novo.
	
	Jedna bitna klasa koja je vezana s prikazivanjem dogaðaja na zaslonu je \lstinline|sf::View|. Ona predstavlja 2D kameru koja definira koji je dio igre prikazan na zaslonu. Pogled možemo koristiti na više naèina. Nudi nam opciju da pomièemo trenutni pogled i tako pokreæemo svijet. Na primjer, ako želimo da se naš lik penje nekamo, možemo pomicati pogled prema gore, a ne sve elemente igre prema dolje. Nudi i sve ostale transformacije iz SFML-a. Takoðer, možemo napraviti dva primjerka klase \lstinline|View| i podijeliti zaslon na dva dijela pa tako imati igru za više igraèa.
	...
	Za uèitavanje i prikazivanje slika u SFML-u primarno su zadužene 3 klase. To su \lstinline{sf::Image} i \lstinline{sf::Texture} (uèitavanje slika) i \lstinline{sf::Sprite} (prikaz slika). Možemo koristiti i oblike za prikazivanje slika, ali zbog svoje jednostavnosti \textit{spriteovi} se èešæe koriste.
	...
	\end{lstlisting}
	Prva dva argumenta predstavljaju širinu i visinu, a treæi predstavlja boju popune. Takoðer kao treæi argument možemo predati niz brojeva tipa \lstinline{sf::Uint8} koji bi predstavljali pojedinaène piksele kao RGB vrijednosti. 
	
	Možemo, naravno, uèitati slike s tvrdog diska.
	\begin{lstlisting}
	sf::Image img;
	...
	Bitan je dio \lstinline{Image} to što nam omoguæava razne funkcije upravljanja slikom, npr. mijenjanje vrijednosti individualnog piksela pomoæu \lstinline{Image::setPixel()}. Nudi nam pristup nizu vrijednosti koje predstavljaju sve piksele ili jednom individualnom pikselu. Sliku možemo okrenuti vertikalno pomoæu \lstinline{Image::flipVertically()} ili horizontalno koristeæi \lstinline{Image::flipHorizontally()}. Sliku na kraju svega možemo spremiti na disk preko funkcije \lstinline{Image::saveToFile()}. 
	
	Nakon što smo upravljali slikama, trebalo bi od njih napraviti teksture. Klasa \lstinline{Texture} nudi nam sliène funkcije kao i \lstinline{Image}. Obje mogu pozivati \lstinline{loadFromFile()} funkciju, postoji samo jedna bitna razlika. U klasi \lstinline{Texture} postoji moguænost prosljeðivanja još jednog argumenta uz ime datoteke, a to su dimenzije i pozicija pravokutnika koji predstavlja samo dio slike. Taj pristup omoguæava postojanje više tekstura za jednu sliku i uèitavanje samo dijelova. To nam štedi vrijeme i memoriju.
	...
	\textit{Game loop} centralni je obrazac za igre koji nam služi za pokretanje igara. Dok \textit{game enginei} sami obavljaju glavnu petlju, SFML nam omoguæava da osobno isprogramiramo glavnu petlju igre. Svaki put kada se odvrti petlja, naš program uèita korisnikov unos, ažurira stanje igre i prikazuje ju korisniku. Ovo je najbitniji oblikovni obrazac kada je u pitanju programiranje igara i skoro svaka igra ga ima.
	...
	\end{lstlisting}
	Vidimo da svaki put kada petlja prolazi, naša igra èita korisnikov unos, ali ako nema unosa, onda \textit{processInput()} neæe ništa napraviti. Funkcija \textit{update()} ažurirat æe stanje na zaslonu neovisno o tome ima li korisnikova unosa ili ne. Ako ima, onda ažurira u skladu s unosom. Na kraju svega toga funkcija \textit{render()} nacrtat æe na zaslon sve potrebno. Dodatno, negdje u kodu bit æe uvjet koji æe omoguæiti izlazak iz ove petlje. Ovo je bio osnovni konceptualni primjer. Imali smo takav u poglavlju koje nas je uvelo u SFML, gdje se u petlji samo èekalo da korisnik zatvori prozor.
	...
	\end{lstlisting}
	Ovdje funkcija \textit{sleep} omoguæava da se petlja ne izvršava preèesto. Sada dolazimo do velike mane ovog pristupa, a to su raèunala kojima æe trebati previše vremena da izvrše petlju. Igra æe tada usporiti i neæe biti željene brzine. Jedno rješenje je da ponudimo korisnicima više opcija za grafiku i detalje u igri tako da petlja traje kraæe, ali uvijek æe postojati još slabija raèunala koja i na najmanjoj rezoluciji neæe moæi postiæi zadovoljavajuæe rezultate. Zato ta opcija i nije najpoželjnija.
	
	\subsubsection{Pomicanje ovisno o vremenu}
	Problem u prošlom rješenju nastajao je kada je trebalo duže vrijeme za obradu jedne slièice. Jedno rješenje toga je pomicanje igre za vrijeme koje nam nedostaje. Dakle, ideja je ta da, ako petlja traje 20 ms, a nama treba više vremena za obradu, onda igru pomaknemo za 20 + višak ms. Toènije, ako se lik pomakne 100 piksela u 20 ms, a nama je potrebno 30 ms, onda æemo pomaknuti lik 150 piksela u jednom izvoðenju petlje.
	...
	Svaku novu slièicu funkcija \lstinline{draw()} briše sve na zaslonu i crta kvadrat. Toènije, ona isprazni \textit{buffer} i nakon toga piše u njega. Tu imamo i funkciju \lstinline{getBuffer()} preko koje \textit{video driver} može pristupiti \textit{bufferu}.
	...
	U zadnjem se primjeru samo èitaju unosi korisnika, ali je dobra ideja ovako pozivati još i \lstinline{update()} i \lstinline{draw()} funkcije jer æemo ovisno o trenutnom stanju pomicati i crtati glavnoga lika. 
	
	\subsection{\zaklj}
	Glavna primjena ovoga obrasca je bolja organizacija koda. Bez njega neke funkcije mogu biti ogromne i neodržive. Ako je funkcija dovoljno velika i ima mnogo \lstinline{if/else} ili prevelik \lstinline{switch}, treba nam \lstinline{State} oblikovni obrazac. Toènije, potreban je ako neki objekt mijenja ponašanje ovisno o stanju. Njime jednostavno mijenjamo stanje glavnoga lika i dodajemo nove dijelove po potrebi. 
	
	
	\chapter{Implementacija igre u SFML-u}

	Ovo poglavlje æe demonstrirati jednu implemetaciju igre u SFML-u. Biti æe pokazana struktura implementacije i neke dobre prakse pri programiranju igara. Igra u pitanju se zove "Crazy Tower". To je 2D platformer u kojem glavni lik skaèe po tornju. Cilj igre je doseæi što viši kat u igri i postiæi što veæi \textit{combo}. U poèetku igraè stoji na poèetnoj platformi i pomicanjem pritiskom na tipke desno i lijevo i skakanjem na tipku \textit{space} pokreæemo našeg lika.
	...
	Kada se lik poène kretati onda se i sama igra nakon par katova poèinje pokretati. Zaslon se kreæe prema gore dok igraè mora ostati iznad dna zaslona. Ako igraè padne ispod dna onda je trenutna igra gotova. Tada se naš najveæi \textit{combo} i najviši kat usporeðuju sa drugim igraèima i stavljaju na ljestvicu. Najviši kat postižemo tako da sa likom skaèemo na sve više katove. Taj rekord nije toliko težak za postiæi koliko je težak visok \textit{combo}. On se postiže tako što igraè preskoèi više od jednog kata u jednom skoku. Tada igraè uðe u \textit{combo} stanje u kojem se svaki put kada preskoèi više od jednog kata brojka preskoèenih katova pribraja samom \textit{combo} broju. Takoðer, imamo vremenski brojaè od 3 sekunde zbog koji se ponovno postavlja svaki put kada poveæamo \textit{combo}. Ako to vrijeme istekne ili ako igraè ne preskoèi više od jednog kata onda se \textit{combo} postavlja na nulu. Iduæa slika pokazuje gdje piše trenutni \textit{combo} \ref{crazyTowerCombo}. Odmah iznad njega je vremenski brojaè za \textit{combo} i iznad njega je sat za poveæanje brzine. On poveæava brzinu kretanja zaslona svakih 30 sekundi i tako èini igru težom što više vremena proðe.
	...
	Igra je implementirana korištenjem više standardnih praksi i oblikovnih obrazaca za igre. Oni služe tome da igra ima kod koji je organiziran, èitljiv i lako se može unaprijediti. Korišteni su obrasci poput \textit{State} i \textit{Game Loop} te neke standardne prakse poput klasa \lstinline|Game| i \lstinline|Player|.
	
	\subsection{Game}
	
	Aplikacije pisane u jeziku C++ poèinju svoje izvršavanje iz \lstinline|main()| funkcije. U igrama je cilj da je što manje logike i programa u glavnoj funkciji. Zbog toga se kao èesta praksa uvede nova klasa \lstinline|Game| koja obavlja svu glavnu logiku igre. Ona je centralna klasa u kojoj cijela igra poèinje svoje izvršavanje. Klasa \lstinline|Game| ima sljedeæu strukturu:
	\begin{lstlisting}
	class    Game
	{
	public:
	...
	\end{lstlisting}
	Ovo je standardna struktura glavne igrine klase. Igra poèinje pozivom funkcije \lstinline|run()| u kojoj se pozivaju ostale igrine funkcije. Ovdje se koristi prvi obrazac u igri, \textsl{Game loop}. Iz glavne petlje poèinje sva centralna logika igre i obrazac koristimo kao što je veæ opisan u jednom od prethodnih poglavlja. Naravno, svu logiku igre neæemo prepustiti ni samoj klasi \lstinline|Game| nego æemo je rasporediti u više klasa koje æe imati specifiène funkcije. U ovom sluèaju u njoj glavna petlja ogranièava igru na 60 slièica po sekundi.
	
	\subsection{Stanja}
	
	Kao i veæina igara ova neæe poèeti tako da igraèa odmah ubaci u igru. Prvo æemo imati glavni zaslon sa nekim opcijama iz kojeg možemo uæi u igru. Za vrijeme igre želimo da postoji moguænost pauziranja igre da igraè može zaustaviti pa nastaviti igru (ili se vratiti na poèetni zaslon). Oèito je sada da su ovo odreðena stanja u kojima se igra može nalaziti pa æemo zbog toga iskoristiti \textsl{State} oblikovni obrazac. Imati æemo više razlièitih stanja u kojima se igra može nalaziti te æemo ih stavljati na stog svih stanja po potrebi. Svako stanje nasljeðuje klasu \lstinline|State| koja ima sljedeæu glavnu strukturu:
	\begin{lstlisting}
	class State
	{
	public:
	...
	\end{lstlisting}
	Sva stanje æe imati funkcije bitne za glavne radnje u igri (crtanje, ažuriranje i dogaðaji). Bitno je primijetiti da ažuriranje i obrada dogaðaja imaju povratni tip \lstinline|bool|. Ovisno o tome što te funkcije vraæaju biti æe odreðeno hoæe li se stanja na stogu ispod njih ažurirati ili ne i obraðivati dogaðaje ili ne. 
	
	Dakle, ideja je ta da stanja stavljamo na stog i onda se ona ažuriraju ovisno o stanjima iznad. Korist ovog pristupa je oèita na primjeru pauziranog stanja. U toj situaciji ne želimo da se igra resetira nego da je u istome stanju koje je bilo prije pauziranja. To znaèi da želimo da se igra iscrtava ali ne i ažurira. To æemo postiæi tako da æe funkcije \lstinline|update| i \lstinline|handleEvent| vratiti vrijednost \lstinline|false| i time naznaèiti stogu da se stanja ispod trenutnog na stogu ne ažuriraju. Kada se igra pauzira onda zapravo gura pauzirano stanje na stog i dolazi do toga da je stanje igre ispod njega. Kada smo gotovi sa pauzom onda samo mièemo element sa vrha stoga i time se nastavlja trenutna igra. Ako se stog isprazni onda znamo da je igra gotova i zatvaramo prozor. Pauzirani zaslon izgleda ovako:
	...
	Sva stanja koja nude neke moguænosti odabira æe sadržavati u sebi jedan izbornik. On æe biti implementiran preko dvije klase, a to su \lstinline|Menu| koja predstavlja jedan izbornik i \lstinline|MenuOption| koja predstavlja jedan izbor. Ona simulira gumb preko klase \lstinline|sf::RectangleShape|. Ovim pristupom je lakše ubaciti izbornike u više razlièitih stanja.
	
	Da bi se razna stanja mogla prikazivati u istom prozoru i da bi mogla meðusobno komunicirati potrebno nam je neka klasa koja bi sadržavala sve bitne informacije. To nam nudi \lstinline|State::Context| koji æe u sebi sadržavati sve objekte koje razna stanja trebaju i dijele. On nam je potreban tako da neke podatke ne uèitamo više puta èime štedimo i procesorsko vrijeme i memoriju. To nam je posebno poželjno kada su u pitanju slike i zvukovi. Njih æemo takoðer spremati na poseban naèin. 
	
	\subsection{Resursi}
	
	Resurse nije praktièno i pregledno držati u zasebnim varijablama. Rastom njihovog broja (što se èesto dogodi u igrama) možemo imati velik broj varijabli i to lako dovodi do nekih bugova. Ono što želimo je neki organizator resursa preko kojih ih možemo uèitati resurse i kasnije im jednostavno pristupiti. Njega bi onda slali stanjima preko konteksta i time osigurali da dodavanje resursa ne zahtjeva velike promjene u kodu. Nakon što uèitamo neki resurs onda æemo mu pridružiti neku identifikaciju (ID) u obliku enumeracije. Primjerak takve klase bi izgledao ovako:
	\begin{lstlisting}
	...
	\end{lstlisting}
	Koristimo \textsl{template} da bi ova klasa bila što opæenitija. Želimo da preko nje možemo pristupiti i teksturama i zvukovima i svim ostalim moguæim resursima. Tako imamo samo jednu klasu za sve resurse. 
	...
	\end{lstlisting}
	Svakoj identifikaciji pripada toèno jedan resurs. Tako je lakše pristupiti potrebnom resursu i kod je manje sklon greškama. U poèetku izvoðenja aplikacije unutar klase \lstinline|Game| uèitaju se svi resursi i onda njima pristupamo preko njihovog ID-a. Kako njih šaljemo unutar konteksta onda æe svako stanje imati pristup.
	
	Ove sve centralne klase nam omoguæuju da imamo dobro organiziran kod koji nije meðusobno ovisan ali održava dobru komunikaciju. Ovakav kostur postavlja dobre temelje za preglednu i lako održivu aplikaciju. 
	
	\subsection{Graf igre}
	
	Najvažniji dio svake igre je onaj u kojoj ju igramo, ostalo su sve dijelovi koji služe da bi on bio bolji. Taj je dio dosta kompliciran za crtanje pa se zato koristi neki malo poželjniji pristup. Kada bi svo crtanje obavljali u jednoj klasi onda to zahtjevalo veliki broj varijabli koje moramo pripremiti i održavati sve na jednom mjestu. Pri svakom dodavanju novog dijela ili mijenjanju starog naš kod je otvoren za greške i sve manje pregledan. Zbog toga æemo crtanje i ažuriranje prebaciti u ævorove grafa naše aplikacije. Taj graf æe biti podijeljen u slojeve i svaki sloj igre imati svoj korijen i djecu koja æe predstavljati dijelove tog sloja. 
	
	Prvo je igra podijeljena u slojeve i to su pozadina, platforme, igraè i zidovi tornja. Svaki od tih slojeva predstavlja korijen grafa tog sloja. Svaki od njih æe imati barem jedno dijete i nekada æe ta djeca imati još djece. U sluèaju sloja sa platformama prvo dijete æe biti klasa \lstinline|Platforms| koja æe pokazivati na više djece tipa \lstinline|Platform| i svaki æe taj èvor predstavljati jednu platformu. Svaka od njih æe se crtati u odnosu na poziciju roditelja, tj. objekta tipa \lstinline|Platforms|. Što znaèi da ako postavimo sve platforme na poziciju (50, 60) i onda stavimo jednu platformu na poziciju (3, 6) onda æe njena pozicija u prozoru biti (53, 66). Dakle dijete radi transformacije relativno na roditelja. 
	
	Ovakav pristup nam omoguæuje da imamo dobru organizaciju koda što se tièe ažuriranja i crtanja te nam omoguæuje da pozicioniramo elemente igre u odnosu na druge. To nam je posebno poželjno ako recimo imamo lika koji se može klonirati i njegov klon ga prati u svakom koraku. Ovakvim pristupom je to jednostavno napraviti samo tako da stavimo da je u grafu klon dijete od lika. Tako æe on lako pratiti poziciju originala.
	...
	Nasljeðivanje \lstinline|Transformable| omoguæuje da nad samim èvorovima radimo transformacije (tj. primjercima klasa koje nasljeðuju \lstinline|SceneNode|), \lstinline|Drawable| omoguæuje da zovemo \lstinline|window.draw(node)| i tako crtamo èvorove.
	
	U njoj možemo dodavati i odmicati djecu èvorova. Pošto se èvorovi transformiraju u odnosu na roditelje onda se nude funkcije \lstinline|getWorldPosition()| i \lstinline|getWorldTransform()| koje nam nude globalne pozicije i transformacije. One služe kada trebamo provjeriti za dva elementa u igri imaju li dodira (stoji li igraè na platformi i sl.). Funkcije \lstinline|update()| i \lstinline|draw()| zapravo pozivaju prvo crtanje i ažuriranje na trenutnom èvoru pa onda na njegovoj djeci, uz to da \lstinline|draw()| prvo primijeni sve transformacije roditelja pa onda crta trenutni èvor. 
	
	Prije svega ovoga postoji klasa \lstinline|Tower| koja prikazuje svijet koristeæi moguænosti klase \lstinline|sf::View|. Ona æe stvoriti korijen grafa koji æe za djecu imati èvorove koji predstavljaju slojeve. Zbog toga æe graf igre izgledati ovako:
	...
	U toj glavnoj klasi se sve svi poèetni èvorovi pozicioniraju po zaslonu i omoguæuje im se meðusobna komunikacija.
	
	Klasa \lstinline|Platforms| služi da inicijaliziranje i ažuriranje svih platformi. Ona organizira platforme i služi kao most za komuniciranje izmeðu platformi i drugih klasa. \lstinline|Platform| predstavlja jednu platformu. \lstinline|RectangleNode| je pozadina igre. \lstinline|TowerWalls| su zidovi desno i lijevo od glavnog dijela igre. Oni na sebi imaju sat koji mjeri kada treba ubrzati kretanje igre, sat koji mjeri trajanje \textit{comboa} i brojaè za \textit{combo}. 
	
	Na kraju \lstinline|Player| klasa predstavlja igraèa. U njoj se analiziraju unosi korisnika koji ga pokreæu i sadržava svu logiku vezanu uz pokrete i ažuriranje pozicije. Ona je centralna klasa u ovom grafu sa kojom svaka druga na neki naèin komuniciraju. Preko nje upravljamo našim igraèem sve dok ne padne ispod zaslona i dobijemo \textit{Game over} zaslon. Na njemu upisujemo naš rezultat ako smo srušili neke rekorde. Cilj je, kao što smo veæ rekli, postiæi što bolji rekord.
	...
	\begin{sazetak}
		U ovom radu vidjeli smo kako funkcionira SFML. To je C++ biblioteka koja omoguæuje programiranje dvodimenzionalnih igara. Ona nudi razne moguænosti za razvoj igara i one su ponuðene preko 5 glavnih modula SFML-a, a to su sustav, prozor, grafika, zvuk i mreža. Sa njima možemo crtati po zaslonu, upravljati likom i ostalim dijelovima programa, puštati zvukove i igrati preko mreže. Sve te moguænosti dovode to toga da možemo programirati kvalitetne i komplicirane igre. Da bi taj kod bio pregledan i održiv koristimo oblikovne obrasce. Oni su isprobane metode organiziranja koda koje dovode do toga da je kvalitetnije napisan. Bez njih bi kodove sa velikim brojem linija bilo teško èitati i održavati. Oni daju razinu apstrakcije kodu tako da nije sve u samo jednoj klasi. Na kraju smo vidjeli kako implementirati jednu igru u SFML-u. Pri tome svo koristili njegove module i oblikovne obrasce. Na tome primjeru se vide moguænosti SFML-a i zašto su korisni obrasci. Takoðer su korištene neke standardne prakse pri programiranju igara koje olakšavaju organizaciju i dodavanje novih dijelova igre.
	\end{sazetak}
	
	\begin{summary}
		In this paper, we have seen the capabilities of SFML. It is a C++ library that allows us to program two-dimensional video games. It offers different possibilities when developing games and they are offered through 5 main modules of SFML, these are system, window, graphics, sound and network. We can use them to draw on the screen, manipulate the character and other parts of the game, play sounds and play over a network. All those possibilities give us the option to develop high quality complex games. To make that code readable and sustainable, we use design patterns. They are tried and tested methods of organizing code that leads to it being written better. Without them, codes that have many lines would be difficult to read and maintain. They provide a level of abstraction to the code so that not all of it is in just one class. Finally, we saw how to implement a game in SFML. While doing so, we used its modules and design patterns. That example showed the capabilities of SFML and usefulness of design patterns. Some standarg game programming practices have also been used which facilitate the organization and addition of new parts to the game.
	\end{summary}
